...
2. **From there, remove the old worktree directory**: `./tools/fork-worktree remove /home/max/Development/rocknix.worktrees/rclone-cleanup` (detached, no build output, nothing unique — its `build/rclone-cleanup` branch is already deleted), then `sudo rm -rf /home/max/Development/rocknix.worktrees` (root-owned 56 KB leftover under `devices/` from a Docker build). The merged `fast-exit-sync` rocknix worktree and the merged ES worktrees were removed on 2026-09-05.
