...
- **Milestone 4 phase 2 (#165) stream (Fable)** building on FEATURE BRANCHES only: distribution `feature/ra-offline` (package `raofflineproxy` pinned `64d03d3063`, `raofflineproxy-ctl`, the unit, backuptool exclusion, suite cases) and ES `feature/ra-offline` from `test/qa-integration` (the OFFLINE RETROACHIEVEMENTS switch with its beta/casual/hardcore-off confirmation, `!RA!` help text, EN+FR). I review, build GENERIC_X64 from the branches, run the VM scenario (#166) before anything merges.
