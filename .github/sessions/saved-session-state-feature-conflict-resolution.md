...
- **Waiting on the maintainer (travelling):** Tailscale access to the RG SP, RG35XX SP and Pocket Nova -> stage H700/SM8550 `3ada78e181` (artifacts `h700-all-20260913-3ada78e181/`, `sm8550-20260913-3ada78e181/`; `scratchpad/stage-h700.sh <host>` shape, each reboot a question), read #161's journal; decisions: #151 Phase 7 close, #47 look, the `!RA!` subtitle (two lines at 640x480: keep or move into the dialog), a game with a RetroAchievements set for #166's unlock proof (else phase 4 on the handheld).
...
- **Drop 2 (milestone 4) state:** phase 2 on feature branches `feature/ra-offline` in both repos (distribution tip `e1edaaa33b` = build-only ES pin over the fixes; ES tip `23dc0336ff`), VM-verified on TEST images `0f7785e6ed`/`e1edaaa33b` (guest d; frames `57307cdf2d`, `f182e15688`; #165 boxes 2-3 ticked; #166 comments). Merge after drop 1's device round: drop the pin commit, merge ES into `test/qa-integration`, merge distribution into `next`, pin the ES tip, build x64 + devices. Owed: 1280x800 frames, H700 package build, #166 unlock proof. Upstream-worthy: the `099-networkservices` variable leak (deletes `smb.conf` too), the proxy's 4xx passthrough patch, the offline-transition log line.
