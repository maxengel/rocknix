...
1. The QoL list in order: #27 (frame on d), then (FTP: missing dir = not there yet; mkdir before the content copy; `--backend ftp` 99/99), #93 + #82 (one ES change, frames), #47 + #27 (frames at 480x320 too), #50 (default hostname per unit; avahi deliberately off?), then the handheld-gated #66/#68/#121. Then one x64 + H700 build carrying everything since `ec12767b26` (#45, the #107 migration, the QoL items), the seven suites plus LINK5, and that becomes the staging candidate in place of `ec12767b26`.
