...
- **SM8550 warm rebuild** from `3ada78e181` (`scratchpad/build-sm8550-3ada78e181.log`; started 17:13 UTC). When OK: copy to `/workspace/artifacts/rocknix-images/sm8550-20260913-3ada78e181/` with checksums (`for t in *.tar; do sha256sum "$t" > "$t.sha256"; cp "$t.sha256" SHA256SUMS.tar; done`), inspect the tar's SYSTEM (`unsquashfs -cat … /etc/os-release`), comment on #150. Then nothing runs until the maintainer's devices are reachable.
