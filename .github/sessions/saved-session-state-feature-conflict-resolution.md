...
- **RC-7 `3387cf5da0` VM-COMPLETE (19:22 UTC):** nine suites PASS (`qa-3387cf5da0-webdav-a-20260914-1854`); fully-offline 0 FAIL; #179 part 1 0 FAIL; walk; index heal PASS (hasher at boot, Tobu healed, Ninoid cached from the index); GAME INDEXES + 1280x800 frames; PL-09 100 games (list within ~1.7 s of A after the tool's latency); #181 recipe as RC-5; upgrade RC-5->RC-7 0 FAIL. H700 RC-7 tar built 18:54-18:57, verified, filed at `h700-all-20260914-3387cf5da0` (sha256 `1f3f8edd48...`) -- **NOT staged; waits for the maintainer's word**. #186: 32/33 (PL-20 upstream); #183 boxes 1-3; #184 all boxes; docs commits `7875dd1b83`, next. Previously (superseded): vm-qa on the pair (`vm-qa-3387cf5da0.log`); `rc7-run.sh` -> `rc6-proofs.sh 3387cf5da0 <img> rebuild fully check1 walk index indexes` on guest d (`rc7/run-1-3387cf5da0.log`): rebuild PASS, fully 0 FAIL; check1/walk/index/indexes pending. Then `rc6-pl09.sh` (100 cached games, summary timing), phases `181`, `upgrade` (RC-5 image -> RC-7 tar). H700 RC-7: `h700/build-h700-3387cf5da0.log`.
...
- **RG SP is on RC-7 `3387cf5da0`** (staged 20:54 UTC under "The device should be visible online now."; rebooted 21:31:14 UTC under "you can reboot then" via `tools/device-act`; back 21:35 on boot id `2d11d638`, queue empty, ES/proxy/tailscaled up, `tailscale.up=1`, offline toggle on, index-at-start on, `ready` 100 -> rising: the link-up top-up is caching the 147 indexed games RC-5's cap had refused; `rgsp-topup-watch.sh` -> `rgsp-topup-3387cf5da0.log` reads the end and the ES lines). Device round next, item by item with a yes each.
...
1. (done) RC-7 VM: read `rc7/run-1-*.log` (index phase = PL-06/PL-08/#183 box 1; `indexes` = GAME INDEXES rows frame); then `rc6-pl09.sh 3387cf5da0 100`, phases `181`, `upgrade`; nine suites' report. Frames -> `docs/qa-frames/2026-09-14/` (+README), QA log row RC-7, work log; tick #186 PL-06/08/09/10, #183 boxes 1/3, #184 3b/5b + "every note has a frame"; upstream-worthy: the `--no-splash` gate is an upstream ES bug (note on #168's list).
2. (done 21:35 UTC) RC-7 applied on the RG SP; read `rgsp-topup-3387cf5da0.log` for the top-up's end and the startup-index lines (#183 on a handheld), then comment on #184 note 6 / #183.
