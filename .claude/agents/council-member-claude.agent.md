...
description: "Council process agent pinned to Claude. Use when running multi-model analysis, peer review, or plan synthesis with the Claude perspective. Invoked by the `council` skill as a member of the default 5-member symmetric roster (degrades to 4 or 3 members when Kimi K3 or Mistral-Large-3 are unreachable)."
...
# the Council Facilitator (`tools/council/council-invoke.ts`) MUST invoke for this seat.
...
# (`tools/council/council-invoke.ts`); the array form remains as a defense
