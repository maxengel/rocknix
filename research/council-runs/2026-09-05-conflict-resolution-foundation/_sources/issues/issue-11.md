...
status:	none
--
Cardinal rule: **preserve player progress, not recency.** A newer write can hold *less* progress than an older one from another device, so newest-wins/mtime is a data-loss trap. Default to **non-destructive** resolution: keep both copies, never auto-delete the loser; prompt/merge over silent overwrite; lean on progress heuristics (playtime/size/state) rather than timestamps.
--
author:	maxengel
association:	owner
edited:	false
status:	none
--
## Kickoff context (2026-09-04)

The phase retro for everything before this milestone is on #26 (comment above, same day). Read it first — its "Discoveries" and "Adjustments" sections are inputs to this milestone's futro. Two decisions taken today: **D-CLOUD-024** (the wizard ships in the current drop) and **D-CLOUD-025** (#19 runs on the bench before the badge is designed). Prior-work grounding: `cloud_device_id` already provides the device identity #20/#21 need; `take_cloud_lock` (`b9ea9f3fe8`) is the serialisation any bisync-based detection (#22) must go through.
--
author:	maxengel
association:	owner
edited:	false
