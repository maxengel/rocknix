...
# The cloud sign-in window needs a compositor it can be mirrored out of: a
# provider's OAuth redirect goes to localhost, so the browser has to run on
# this device, and the player has to be able to see and type into it from a
# phone. weston's VNC backend serves that directly -- no second VNC server,
# and no claim on KMS, so whatever is already on the screen stays there.
#
# Appended rather than edited into the block above so this stays one line of
# difference from upstream's option set.
