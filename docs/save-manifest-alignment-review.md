...
| D-CLOUD-005 | the settings-backup upload decides on content, not remote metadata | `sha256` is the identity; `remote_hash`/`mtime` are shortcuts, never the decision (§2, §6) | aligned, same principle |
...
| D-CLOUD-018 / 019 / 020 | content layout `ROMs/`+`BIOS/`; content membership is an allowlist; three tiers by size and cadence | manifest lives inside the **saves** tier only: under `savestates/` (saves tier), never under `CONTENT_REMOTE`, never in `backuptool`'s archive (`/storage/.config/*`; `/storage/.cache` and `/storage/roms` are not archived — verified in `backuptool` DEFAULT) | aligned |
...
| #12 directory chooser, #28 sync paths, #33 REMOTENAME | `SAVES_REMOTE` and the remote are user-chosen | entry keys are relative to the sync root, so a `SAVES_REMOTE` change moves nothing; the manifest names no remote. `SAVESPATH` and `SAVESPATH` are both `/storage/roms` by default and **the schema assumes they stay equal** — if a player set them apart, "relative to the sync root" is two roots. Documented assumption (§3.4) |
...
4. **Documented assumption, now enforced:** saves have one local folder, `SAVESPATH`; the old `RESTOREPATH` is removed and a config still carrying a different value makes every saves transfer refuse (D-CLOUD-040). Was: `BACKUPPATH == RESTOREPATH`, true by default and in every shipped flow; a player who sets them apart has two sync roots, and entry keys are relative to one. `cloud_sync_helper` could warn; not worth more than a line in the schema until somebody does it.
...
- The manifest can never enter the content tier or the settings-backup archive.
